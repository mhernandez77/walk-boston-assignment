function renderPosts(boston, container) {
    ...
    for (let i = 0; i < len; i++) {
        html +=
            '<li class="post">' + '<h2>' + people[i][8] +
            '</h2>' + '<h3>' + people[i][11] + '</h3>';
    }
    container.innerHTML = '<ul id = "data">' + html + '</ul>';
}

// TODO: add code to display the html variable inside a ul
// element with id="data"
// Hint: you can use the container parameter's innerHTML property
//  to insert Html tags
